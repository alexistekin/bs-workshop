  ...
  constructor(parent: Construct, name: string, props: RoleConstructProps) {
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:cloudformation:*:${props.aadEnv.awsAccount}:stack/${props.aadEnv.prefix}-api-*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        actions: ["states:*"],
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:states:*:${props.aadEnv.awsAccount}:*`],
      })
    );
    ...
    for (const vpc of props.vpcCollection) {
      this.IAMRole.addToPolicy(
        new iam.PolicyStatement({
          actions: ["ec2:*"],
          effect: iam.Effect.ALLOW,
          resources: [`arn:aws:ec2:*:${props.aadEnv.awsAccount}:vpc/${vpc.vpcId}`],
        })
      );
    }
    ...
    this.IAMRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        effect: iam.Effect.ALLOW,
        resources: [`arn:aws:dynamodb:*:${props.aadEnv.awsAccount}:*`],
      })
    );
    ...
    this.IAMRole.addToPolicy(
     ...
     const roleParam = new ssm.StringParameter(this, `${envIdentifier}-role-param`, {
      allowedPattern: ".*",
      description: `The Role for AAD Solution: ${props.aadEnv.envName} Environment`,
    ...
    const roleArnParam = new ssm.StringParameter(this, `${envIdentifier}-role-arn-param`, {
      allowedPattern: ".*",
      description: `The Role Arn for AAD Solution: ${props.aadEnv.envName} Environment`,
