...
do
  ...
  GITLAB_URL=`aws ssm get-parameter --name "/aad/gitlab-url" --output text --query Parameter.Value --with-decryption`
  if [[ -z "$GITLAB_URL" ]]; then
    echo "Sleeping for 20 seconds"
    sleep 20
  else
    echo "GITLAB_URL is \"$GITLAB_URL\""
    ADMIN_USERNAME=$(aws secretsmanager get-secret-value --secret-id aad-admin-gitlab-secrets| jq --raw-output '.SecretString' | jq -r .username)
    ...
    aws secretsmanager put-secret-value --secret-id aad-admin-gitlab-secrets --secret-string '{"apiToken":"'"$ADMIN_TOKEN"'", "password":"'"$ADMIN_GITLAB_PASSWORD"'", "username":"'"$ADMIN_USERNAME"'", "runnerRegistrationToken":"", "runnerId":""}'
    ...
    if [[ ! -z "$RUNNER_TOKEN" ]]; then
        ...
        aws secretsmanager put-secret-value --secret-id aad-admin-gitlab-secrets --secret-string '{"apiToken":"'"$ADMIN_TOKEN"'", "password":"'"$ADMIN_GITLAB_PASSWORD"'", "username":"'"$ADMIN_USERNAME"'", "runnerRegistrationToken":"'"$RUNNER_TOKEN"'", "runnerId":""}'
    else
