...
export class GitlabRunnerConstruct extends Construct {
  constructor(parent: Construct, name: string, props: GitlabRunnerConstructProps) {
    ...
    const glRunnerEc2SgName = `${props.aadEnv.prefix}-gitlab-runner-sg`;
    ...
    const gitlabIamRolePolicy = new iam.PolicyDocument({
      statements: [
        ...
        new iam.PolicyStatement({
          ...
          effect: iam.Effect.ALLOW,
          resources: ["*"],
          // !FIXME: add condition to only allow role assumption when running in a pipeline
        }),
    ...
    const roleParam = new ssm.StringParameter(this, `${props.aadEnv.prefix}-pipeline-role`, {
      allowedPattern: ".*",
      description: `The AAD Platform Pipeline Role Arn`,
      parameterName: `/${props.aadEnv.prefix}/pipeline-role`,
      stringValue: gitlabEc2Role.roleArn,
    });

    new cdk.CfnOutput(this, `The AAD Platform Pipeline Role Arn Parameter`, {
