...
import { AADEnvironmentParams, HostedZoneConstruct, NetworkConstruct } from "@aws-app-development/common-constructs";
...
export class GitlabHostingConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: GitlabHostingConstructProps) {
    ...
    new ssm.StringParameter(this, `${props.aadEnv.prefix}-gitlab-security-group`, {
      allowedPattern: ".*",
      description: `The Gitlab LB Security Group`,
      parameterName: `/${props.aadEnv.prefix}/gitlab-security-group`,
      stringValue: albSecurityGroup.securityGroupId,
    });

    const glRunnerSgName = `${props.aadEnv.prefix}-alb-gitlab-runner-sg`;
    ...
    const gitlabHost = new ec2.Instance(this, "GitlabHost", {
      instanceName: `${props.aadEnv.prefix}-GitlabHost`,
    ...
    // create ALB
    const gitlabAlb = new elb.ApplicationLoadBalancer(this, `${props.aadEnv.prefix}-gitlab-alb`, {
      vpc: props.network.vpc,
      vpcSubnets: { subnetType: ec2.SubnetType.PUBLIC },
      loadBalancerName: `${props.aadEnv.prefix}-gitlab-alb`,
    ...
    if (props.secureDeployment) {
      // Setup nice A Record to access gitlab
      const aRecord = new route53.ARecord(this, `${props.aadEnv.prefix}-r53-aliasrecord`, {
      ...
      const httpsListener = gitlabAlb.addListener(`${props.aadEnv.prefix}-alb-httpsListener`, {
        protocol: elb.ApplicationProtocol.HTTPS,
        open: false,
        certificates: [props.hostedZone?.certificate!],
      });

      httpsListener.addTargets(`${props.aadEnv.prefix}-target-grp2`, {
        protocol: elb.ApplicationProtocol.HTTP,
        targetGroupName: `${props.aadEnv.prefix}-git-target-grp2`,
      ...
      lbDNSParam = new ssm.StringParameter(this, `${props.aadEnv.prefix}-gitlab-hostname`, {
        allowedPattern: ".*",
        description: `The Gitlab Hostname for AAD Platform`,
        parameterName: `/${props.aadEnv.prefix}/gitlab-hostname`,
        stringValue: aRecord.domainName,
      });

      lbUrlParam = new ssm.StringParameter(this, `${props.aadEnv.prefix}-gitlab-url`, {
        allowedPattern: ".*",
        description: `The Gitlab URL for AAD Platform`,
        parameterName: `/${props.aadEnv.prefix}/gitlab-url`,
      ...
      // non secure port
      const httpListener = gitlabAlb.addListener(`${props.aadEnv.prefix}-alb-httpListener`, {
        protocol: elb.ApplicationProtocol.HTTP,
        port: 80,
      });

      httpListener.addTargets(`${props.aadEnv.prefix}-target-grp`, {
        protocol: elb.ApplicationProtocol.HTTP,
        targetGroupName: `${props.aadEnv.prefix}-git-target-grp`,
        targets: [new elbTargets.InstanceTarget(gitlabHost, 80)],
        healthCheck: {
          enabled: true,
          healthyHttpCodes: "302",
      ...
      httpListener.connections.addSecurityGroup(props.network.allowedIpsSg);
      ...
      lbDNSParam = new ssm.StringParameter(this, `${props.aadEnv.prefix}-gitlab-hostname`, {
        ...
        description: `The Gitlab Hostname for AAD Platform`,
        parameterName: `/${props.aadEnv.prefix}/gitlab-hostname`,
        stringValue: gitlabAlb.loadBalancerDnsName,
      });
      ...
      lbUrlParam = new ssm.StringParameter(this, `${props.aadEnv.prefix}-gitlab-url`, {
        ...
        description: `The Gitlab URL for AAD Platform`,
        parameterName: `/${props.aadEnv.prefix}/gitlab-url`,
        stringValue: "http://" + gitlabAlb.loadBalancerDnsName,
      });

      this.gitlabHostName = gitlabAlb.loadBalancerDnsName;
    }
