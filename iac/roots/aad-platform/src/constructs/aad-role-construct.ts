...
import { AADEnvironmentParams, NetworkConstruct } from "@aws-app-development/common-constructs";
...
export interface AADRootRoleConstructProps extends cdk.StackProps {
...
const defaultProps: Partial<AADRootRoleConstructProps> = {};

export class AADRootRoleConstruct extends Construct {
  public IAMRole: iam.Role;
  private props: AADRootRoleConstructProps;

  constructor(parent: Construct, name: string, props: AADRootRoleConstructProps) {
    super(parent, name);
    ...
    this.IAMRole = new iam.Role(this, name, {
      ...
      inlinePolicies: {
        [`${name}-role-assumption`]: this.createRoleAssumptionPolicyDocument(this.props.aadEnv.awsAccount),
      },
    ...
    const roleParam = new ssm.StringParameter(this, `${this.props.aadEnv.prefix}-platform-role`, {
      allowedPattern: ".*",
      description: `The AAD Backstage Platform Role Arn`,
      parameterName: `/${this.props.aadEnv.prefix}/platform-role`,
  ...
  private createSecretsManagerStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      resources: [`arn:aws:secretsmanager:*:${this.props.aadEnv.awsAccount}:secret:*`],
    });
  }
  ...
  private createS3BucketStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      conditions: {
        StringEquals: {
          "aws:ResourceAccount": this.props.aadEnv.awsAccount,
        },
      },
    });
  }
  ...
  private createResourceGroupsStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:resource-groups:*:${this.props.aadEnv.awsAccount}:group/*`],
    });
  }
  ...
  private createCloudformationStatement(): cdk.aws_iam.PolicyStatement {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:cloudformation:*:${this.props.aadEnv.awsAccount}:stack/${this.props.aadEnv.prefix}-api-*`],
    });
  }
  ...
  private createEc2Statement() {
    return new iam.PolicyStatement({
      actions: ["ec2:*"],
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:ec2:*:${this.props.aadEnv.awsAccount}:vpc/${this.props.network.vpc.vpcId}`],
    });
  }
  ...
  private createDyamodbStatement() {
    return new iam.PolicyStatement({
      ...
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:dynamodb:*:${this.props.aadEnv.awsAccount}:table/${this.props.securityTableName}`],
    });
  }
  ...
  private createRoleAssumptionPolicyDocument(account: string): cdk.aws_iam.PolicyDocument {
    ...
    const aadRoleAssumptionPolicy = new iam.PolicyStatement({
      sid: "AllowAADRoleAssumptionInAccount",
      actions: ["sts:AssumeRole"],
      effect: iam.Effect.ALLOW,
      resources: [`arn:aws:iam::${account}:role/aad*`],
    });
