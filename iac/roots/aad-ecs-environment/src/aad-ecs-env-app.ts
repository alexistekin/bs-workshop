...
import { AADECSEnvStack } from "./aad-ecs-environment-stack";
...
async function main() {
  ...
  new AADECSEnvStack(app, `ECS-ENV-${prefix}-${process.env.ENV_NAME}-Stack`, {
    // stackName: `aad-ecs-environment`,  // Do not use stack name to get a generated stack name so multiple stacks can be created
    description: `${envID} ECS Environment for AAD(AWS App Development)`,
