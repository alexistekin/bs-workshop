...
if [[ -z "$ALREADY_DEPENDS_ON" ]]; then yq -Yi ".spec.dependsOn += [\"${RESOURCE_ENTITY_REF}\"]" catalog-info.yaml; fi
...
git status
if [[ "$UPDATE_COUNT" -gt "0" ]]; then git commit -m "Bind Resource to env ${TARGET_ENV_NAME}-${TARGET_ENV_PROVIDER_NAME}" --quiet; fi
