...
do
    ...
    if [[ "$AAD_CI_ENVIRONMENT_MANUAL_APPROVAL"  == "true" ]]; then
    ...
    if [[ ! -f "$STACK_FILE_PATH" ]]; then
        ...
        sed -i "s|VpcIdValue|$TARGET_VPCID|g" $STACK_FILE_PATH
        sed -i "s|VpcSubnetIdsValue|$TARGET_PRIVATE_SUBNETS|g" $STACK_FILE_PATH
