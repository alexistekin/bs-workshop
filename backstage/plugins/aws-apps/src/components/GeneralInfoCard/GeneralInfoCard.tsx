...
const BawsAppGeneralInfo = ({
...
}: { input: { account: string, region: string, gitHostUrl: string, gitApp: string, repoSecretArn: string, api:AADApi, appPending:boolean } }) => {
  ...
  async function getData() {
    ...
    const gitHostParam =  await api.getPlatformSSMParam({paramName: "/aad/gitlab-hostname"});
...
export const GeneralInfoCard = ({appPending}:{appPending:boolean}) => {
  ...
  if (appPending)
  {
    api.setBackendParams({
      ...
      awsAccount:'',
      awsRegion:config.getString('backend.platformRegion'),
      prefix: '',
      providerName: ''
    });
    const input = {
  ...
  else
  {
    if (awsAppLoadingStatus.loading) {
      ...
      api.setBackendParams({
        appName:awsAppLoadingStatus.component.componentName,
        awsAccount:awsAppLoadingStatus.component.currentEnvironment.providerData.accountNumber,
        awsRegion:awsAppLoadingStatus.component.currentEnvironment.providerData.region,
        prefix: awsAppLoadingStatus.component.currentEnvironment.providerData.prefix,
        providerName: awsAppLoadingStatus.component.currentEnvironment.providerData.name
      });
